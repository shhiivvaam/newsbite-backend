...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (category) {
      ...
      if (!cat) return res.status(400).json({ error: 'Invalid category name' });
    }
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (data.category) {
      const cat = await prisma.categories.findUnique({ where: { name: data.category } });
      if (!cat) return res.status(400).json({ error: 'Invalid category name' });
    }
